...
extension OfferListCellViewModel {
    ...
    static func mockOffers() -> [Offer] {
        var offers = [Offer]()
        for _ in 1...5 {
            offers.append(mockOffer())
        }
        return offers
    }
    static func mockVM() -> OfferListCellViewModel {
        let mockOffer = self.mockOffer()
        return OfferListCellViewModel(offer: mockOffer)
    }
}
